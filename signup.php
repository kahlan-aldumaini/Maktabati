...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($_POST["email"]) || empty($_POST["password"]) || empty($_POST["username"]) || !isset($_POST['checkbox'])) {
        $result = " <div class='alert alert-danger'>" . lang('signup_empty_field') . "</div>";
    } else {
        ...
        $userName = strip_tags($usern);
        // filter email from any script 
        $userEmail = filter_var($usere, FILTER_VALIDATE_EMAIL, FILTER_NULL_ON_FAILURE);
        if (!empty($userName) && !empty($userEmail) && !empty($password)) {
            ...
            if ($results != 0) {
                $result = '<div class="alert alert-danger">' . lang('error_email_exist') . '</div>';
            } else {
                // Insert  new User 
...
<div class="mt-5 loginpage signuppage d-flex align-items-center justify-content-center">
        ...
        <div class="container my-5 shadow d-flex align-items-center justify-content-center">
            <div class="row bg-white">
                <div class="col text-primary p-3">
                    ...
                    <hr class="mb-2" />
                    <span class="text-danger p-auto"><?php if (!empty($result)) {
                                                            echo $result;
                                                        } ?></span>
                    <main class="all-form m-4">
                        <form name="login" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="POST">
                            ...
                            <div class="form-group abs-con">
                                ...
                                <input type="text" class="form-control" id="email" onkeyup="checkSignupData('#check_email')" onblur="checkValid()" name="email" placeholder="<?php echo lang('email_placeholder') ?>" required>
                            </div>
                            <small class="col ps-4 mb-3 pe-4 text-center" id="check_email" style="width:0rem;"></small>

                            <div class="form-group abs-con mb-2">
                                <label for="password" class="form-label"><?= lang('password'); ?></label>
                                <span class="start-icon"><i class="fa-duotone fa-lock"></i></span>
                                <input type="password" class="form-control" id="password" onkeyup="checkSignupData('#valid_pass')" oninput="checkValid()" name="password" placeholder="<?php echo lang('password_placeholder') ?>" required>
                            ...
                            <small class="col ps-4 mb-2 pe-4 text-center" id="valid_pass" style="width:0rem;"></small>
                            <div class="form-group abs-con">
                                <input class="form-check-input" name="checkbox" type="checkbox" value="check" id="checkbox" onclick="ifCheckBox()" />
                            ...
                            <div class="d-grid gap-2">
                                <button class="btn blue-btn rounded-3 mt-2 mb-2 fw-bold" type="submit" id="signup_btn" disabled><?php echo lang('create_account') ?></button>
                            </div>
